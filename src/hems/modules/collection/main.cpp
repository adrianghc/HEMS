...
int main(int argc, char* argv[]) {
    using namespace hems::modules::collection;
    ...
    hems::logger::this_logger = new hems::remote_logger(hems_collection::module_type, debug);
    ...
    try {
        hems_collection::this_instance = new hems_collection(false);
        ...
        hems::logger::this_logger->log(
            "FATAL: Could not start instance of " +
                hems::modules::to_string_extended(hems_collection::module_type) + ".",
            hems::logger::level::ERR
        );
        hems::exit(err);
    }

    delete hems_collection::this_instance;
