/*
 * This is a HTTP worker implementation, slightly modified from the original lifted out of
 * https://www.boost.org/doc/libs/1_75_0/libs/beast/example/http/server/fast/http_server_fast.cpp.
 * 
...
#ifndef EXTRAS_HTTP_WORKER_HPP
#define EXTRAS_HTTP_WORKER_HPP
...
#include "extras/fields_alloc.hpp"
...
class http_worker {
    public:
        ...
        http_worker(
            tcp::acceptor& acceptor, const std::string& doc_root, const std::string& default_res,
            const std::function<std::tuple<const std::string, const std::string, bool>(
                const std::string&, const std::string&
            )> post_cb
        ) : acceptor_(acceptor), doc_root_(doc_root), default_res_(default_res), post_callback(post_cb) {}
    ...
    private:
        ...
        /* The name of the default resource to return if none is explicitly provided. */
        std::string default_res_;
        ...
        /* The callback for POST requests. */
        const std::function<std::tuple<const std::string, const std::string, bool>(
            const std::string&, const std::string&
        )>& post_callback;
        ...
        void read_request() {
            /**
             * On each read the parser needs to be destroyed and recreated. We store it in a
             * boost::optional to achieve that.
             * 
             * Arguments passed to the parser constructor are forwarded to the message object. A
             * single argument is forwarded to the body constructor.
             * 
             * We construct the dynamic body with a 1MB limit to prevent vulnerability to buffer
             * attacks.
             */
        ...
        void process_request(http::request<request_body_t, http::basic_fields<alloc_t>> const& req) {
            beast::string_view target = req.target() == "/" ? default_res_ : req.target();
            switch (req.method()) {
                case http::verb::get: {
                    send_file(target);
                    break;
                ...
                case http::verb::post: {
                    std::tuple<const std::string, const std::string, bool> res =
                        post_callback(req.body(), std::string(target));

                    if (std::get<2>(res)) {
                        send_file(std::get<0>(res));
                    } else {
                        send_good_response(std::get<0>(res), std::get<1>(res));
                    }
                    break;
                }
                default: {
                    /*  We return responses indicating an error if we do not recognize the request
                        method. */
                    send_bad_response(
        ...
        void send_good_response(std::string const& response, std::string const& mime_type) {
            string_response_.emplace(std::piecewise_construct, std::make_tuple(), std::make_tuple(alloc_));

            string_response_->result(http::status::ok);
            string_response_->keep_alive(false);
            string_response_->set(http::field::server, "Beast");
            string_response_->set(http::field::content_type, mime_type);
            string_response_->body() = response;
            string_response_->prepare_payload();
